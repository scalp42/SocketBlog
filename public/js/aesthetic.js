aesthetic = {
  ...
  loadimage: function(url) {
    ...
    $('#holder').remove();
    $('body')
      .append($('<img>').attr('id', 'holder'))
      .append($('<div></div>').attr('id', 'sourceCanvas'))
      .append($('<div></div>').attr('id', 'hiddenStuff'));

    //  add the load event, so when the BASE64 comes back from the server
    //  the next function will kick in.
    $('img#holder').load(function(){
      aesthetic.copyToCanvas();
    });

    //  set the image data
    $('img#holder').attr('src', url);
  },
